...
<img src="https://i.loli.net/2021/08/02/TIntaSK98VGANUd.jpg" alt="Data Engineering-28" style="zoom:50%;" />
...
* API is literally a way of interacting with the service in order to get some information or to enter some information into the service. 
* SDK is a tool and a package to actually create an application. You can install it to start building an app. It is available in specific programming language like Java or python (ex. Java SDK, Python SDK) and also it may be for a specific part of the service (ex. web playback SDK in Spotify is SDK specifically for the web playback feature only). 
* Therefore, SDKs often contain APIs, but they make it more high level for developers to use. 

REST API, what is RESTful ? 

* Representational state transfer
* REST is an architectural style. It defines a set of constraints on how the web based services should behave. 
* REST architecture uses HTTP protocols to communicate between REST client and REST server.
* Stateless client-server communication : each request is independent. Requests do not depend on past requests. 
