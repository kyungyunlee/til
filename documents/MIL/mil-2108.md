...
## 8/21
### AWS IAM 
AWS 리소스에 대한 접근 권한을 관리해주는 서비스. 
* IAM user: AWS 리소스를 사용하는 유저 
* IAM group: 유저들의 그룹 
* IAM role: role을 만들면 유저에 할당할 수 있음.
* IAM policy: user, group, role의 권한을 설명해주는 오브젝트 

IAM은 글로벌 리소스이다. 그래서 region에 상관없이 만들어지는 것이라 하나를 만들어서 모든 region에서 사용하는 것이다. 

Policy 는 or 정책으로 이루어지고 deny는 우선적으로 적용한다.  
AWS cross account?? 는 and 정책으로 이뤄진다. 

aws_iam_instace_profile ? 
